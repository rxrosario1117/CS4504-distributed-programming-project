...
public class SThread extends Thread {
	 ...
	 * Constructor
	 */
	SThread(Object[][] Table, Socket toClient, int index, String SenderNickname, String ReceivNickname)
			throws IOException {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				if (destination.equals((String) RTable[i][0])) {
					...
					outTo = new PrintWriter(outSocket.getOutputStream(), true); // assignigns a writer
				}
			}

			// Communication loop
			// while ((inputLineine = in.readLine()) != null) {
			// System.out.println("Client/Server said: " + inputLine);

			// outputLine = inputLine; // passes the input from the machine to the output
			...
			System.err.println("Could not lt listen to socket.");
			System.exit(1);
		}
	}
